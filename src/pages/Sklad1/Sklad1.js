...
const Sklad1 = ({
  ...
  const dayNow = new Date().getFullYear() + '-' + new Date().getMonth() + '-' + (new Date().getDate() < 10 ? '0' + new Date().getDate() : new Date().getDate());
  const h = useHistory();
        ...
        <div className={s.title__container}>
          ...
          || activeTabIndex === 2 && (
            <>
              ...
              <div className={s.search__container}>
                <Input label="На дату" type="date" value={dayNow} onChange={({target}) => {
                  setDataForFilter({day: target.value})
                }}/>
              </div>
              <div className={s.create__worker}>
