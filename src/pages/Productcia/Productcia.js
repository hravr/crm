...
const Productcia = ({
    ...
    const patchSingleProdArticle = (id) => {
        ...
        console.log('patchSingleProdArticle === ', singleArticle);

        patchProdArticle(singleArticle._id, token, singleArticle)
            .then((res) => {
                    res.status === 200 && setProdArticle(prevState => prevState.filter(article => article._id === singleArticle._id ? article.name = singleArticle.name : article))
                }
            );
    };
    ...
    const getSingleProdArticle = (id) => {
        const token = getToken();
        fetchSingleProdArticle(id, token)
            .then((res) => {
